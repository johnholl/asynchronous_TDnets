...
parser.add_argument('-e', '--agent-id', type=str, default="A3C",
                    help="Agent id")
parser.add_argument('-l', '--log-dir', type=str, default="/tmp/experiment",
...
def create_commands(session, num_workers, remotes, agent_id, logdir, shell='bash', mode='tmux'):
    # for launching the TF workers and for launching tensorboard
    base_cmd = [
        'CUDA_VISIBLE_DEVICES=',
        sys.executable, 'helpers/worker.py',
        '--log-dir', logdir,
        '--agent-id', agent_id,
...
def run():
    ...
    cmds, notes = create_commands("a3c", args.num_workers, args.remotes, args.agent_id, args.log_dir, mode=args.mode)
    if args.dry_run:
