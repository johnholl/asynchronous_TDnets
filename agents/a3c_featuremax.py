...
from models.model import A3CLSTMPolicy
...
"""
        ...
        with tf.device(tf.train.replica_device_setter(1, worker_device=worker_device)):
            with tf.variable_scope("global"):
                self.network = A3CLSTMPolicy(env.observation_space_shape, env.num_actions)
                self.global_step = tf.get_variable("global_step", [], tf.int32, initializer=tf.constant_initializer(0, dtype=tf.int32),
                                                   trainable=False)

        with tf.device(worker_device):
            with tf.variable_scope("local"):
                self.local_network = pi = A3CLSTMPolicy(env.observation_space_shape, env.num_actions)
            ...
            entropy_reg = 0.001
            self.loss = pi_loss + 0.5 * vf_loss - entropy * entropy_reg
