...
MC_pix_dict = load_obj("MC_10000_det_pix_seed0")
...
"""
    def __init__(self, prev_action=[], prev_reward=[], states=[], actions=[], rewards=[], terminal=[], features=[]):
        ...
        # self.last_pix_changes = []

    def add(self, state, action, reward, terminal, prev_action, prev_reward):
        self.prev_action += [prev_action]
        self.prev_reward += [prev_reward]
        self.states += [state]
        self.actions += [action]
        ...
        # self.last_pix_changes += [last_pix_change]
    ...
    def extend(self, other):
        ...
        # self.last_pix_changes.extend(other.last_pix_changes)
...
def env_runner(env, policy, beh_policy, num_local_steps, summary_writer):
    """
The logic of the thread runner.  In brief, it constantly keeps on running
the policy, and as long as the rollout exceeds a certain length, the thread
runner appends the policy to the queue.
"""
    last_state = env.reset(seed=0)
    ...
    while True:
        ...
        for _ in range(num_local_steps):
            ...
            #
            # if not terminal:
            #     next_value, next_prediction = policy.value(state, action, reward, prediction)
            #
            # else:
            #     next_value = 0.
            ...
            rollout.add(last_state, action, reward, terminal, prev_action, [prev_reward])
            ...
            if terminal:
                ...
                terminal_end = True
                for i in range(5):
                    rollout.add(np.zeros(shape=[84,84,3]), prev_action, 0., terminal, prev_action, [reward])
                last_state = env.reset(seed=0)
                last_beh_features = beh_policy.get_initial_features()
                # last_prediction = policy.get_initial_features()
        ...
        if not terminal_end:
            rollout.r = value_
            next_rollout = PartialRollout(prev_action=rollout.prev_action[-5:], prev_reward=rollout.prev_reward[-5:],
                                      states=rollout.states[-5:], rewards=rollout.rewards[-5:],
                                          terminal=rollout.terminal[-5:], actions=rollout.actions[-5:])
        else:
            next_rollout = PartialRollout()


class ConvTDAgent(object):
...
"""
        ...
        with tf.device(worker_device):
            ...
            self.value_loss = tf.reduce_sum(tf.where(
                                    tf.abs(val_delta) < 1,
            ...
            pix_delta = pi.approx_predictions - pi.target_predictions

            self.prediction_loss = prediction_loss_weight*tf.reduce_sum(tf.where(
                                    tf.abs(pix_delta) < 1,
            ...
            self.loss = self.value_loss + self.prediction_loss
            ...
            agent_grads = tf.gradients(self.value_loss, pi.var_list)
            ...
            tf.summary.scalar("model/Q_loss", self.value_loss / bs)
            ...
            self.predloss_sum = tf.summary.scalar("model/pixel_loss", self.prediction_loss / bs)
...
"""
        ...
        if should_compute_summary:
            fetches = [self.summary_op, self.predmax, self.predval, self.agent_train_op, self.global_step]
        else:
        ...
        # quantities that are needed from 20 step unroll to train question network
        # (1) next n rewards
        # (2) value at the end of that
        # (3) observations arranged appropriately as [20, 5, 3, 84, 84]
        # (4) current observations
        #  This indicates that during env_runner we don't need to compute values. Also, need to figure out how
        #  to arrange things so that we have access to next n observations at this time.
        #
        ...
        batch_pr = np.asarray(rollout.prev_reward)
        batch_si = np.asarray(rollout.states)
        last_pred = sess.run(self.local_network.default_last_prediction, feed_dict={self.local_network.bs: 1})

        print("VALUE TARGET SHAPES")
        print(np.shape(last_pred))
        print(np.shape([batch_si[-6]]))
        print(np.shape([batch_si[-5:]]))
        print(np.shape([batch_pr[-6]]))
        print(np.shape([batch_pa[-6]]))



        final_val = sess.run(self.local_network.val,
                           feed_dict={self.local_network.x: [batch_si[-6]], self.local_network.target_obs: [batch_si[-5:]],
                                      self.local_network.reward: [batch_pr[-6]], self.local_network.action: [batch_pa[-6]],
                                      self.local_network.use_target: True,
                                      self.local_network.bs: 1,
                                      self.local_network.last_prediction: last_pred})
        print(np.shape(batch_rewards))
        value_targets = discount(batch_rewards[:-5], .99)
        for i in range(len(batch_si)-5):
            value_targets[i] += (.99**(len(batch_si)-5-i))*final_val

        future_obs = [] # put it together using list of states
        for i in range(len(batch_si)-5):
            next_obsseq = batch_si[i+1:i+6]
            future_obs.append(next_obsseq)
        ...
        # print(batch_val)
        # print(value_targets)
        last_preds = sess.run(self.local_network.default_last_prediction, feed_dict={self.local_network.bs: len(batch_si)-5})
        feed_dict = {
            self.local_network.x: batch_si[:-5],
            self.local_network.action: batch_pa[:-5],
            self.local_network.reward: batch_pr[:-5],
            self.val_target: value_targets,
            self.local_network.bs: len(batch_si)-5,
            # self.ground_value: [MC_value_dict[str(state)][1] for state in batch_si[:-5]],
            self.local_network.target_obs: future_obs,
            self.local_network.use_target: True,
            self.local_network.last_prediction: last_preds
        }

        # print(np.shape(batch_si[:-5]))
        # print(np.shape(batch_pa[:-5]))
        # print(np.shape(batch_pr[:-5]))
        # print(np.shape(value_targets))
        # print(np.shape(future_obs))
        # print(np.shape(last_preds))
