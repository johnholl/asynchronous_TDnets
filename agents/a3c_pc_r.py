...
from models.model import GridPredictionLSTMPolicy
...
"""
        ...
        with tf.device(tf.train.replica_device_setter(1, worker_device=worker_device)):
            with tf.variable_scope("global"):
                self.network = GridPredictionLSTMPolicy(env.observation_space_shape, env.num_actions, grid_size=grid_size)
                self.global_step = tf.get_variable("global_step", [], tf.int32, initializer=tf.constant_initializer(0, dtype=tf.int32),
                                                   trainable=False)

        with tf.device(worker_device):
            with tf.variable_scope("local"):
                self.local_network = pi = GridPredictionLSTMPolicy(env.observation_space_shape, env.num_actions, grid_size=grid_size)
...
"""
        ...
        feed_dict = {
            ...
            self.local_network.state_in[2]: batch.features[2]
        }
