...
from models.model import FeatureLSTMPolicy
...
"""
    ...
    last_conv_features = np.zeros(shape=[9,9,32])
    length = 0
    rewards = 0
    prev_reward = 0
    ...
    while True:
        ...
        for _ in range(num_local_steps):
            ...
            action, value_, conv_features, features = fetched[0], fetched[1], fetched[2], fetched[3:]
            ...
            feat_change = np.squeeze(np.abs(last_conv_features - conv_features), axis=0)
...
"""
        ...
        with tf.device(tf.train.replica_device_setter(1, worker_device=worker_device)):
            with tf.variable_scope("global"):
                self.network = FeatureLSTMPolicy(env.observation_space_shape, env.num_actions, mode="Features")
                self.global_step = tf.get_variable("global_step", [], tf.int32, initializer=tf.constant_initializer(0, dtype=tf.int32),
                                                   trainable=False)

        with tf.device(worker_device):
            with tf.variable_scope("local"):
                self.local_network = pi = FeatureLSTMPolicy(env.observation_space_shape, env.num_actions, mode="Features")
            ...
            self.prediction_target = tf.placeholder(tf.float32, [None, 9, 9, 32])
            self.action = tf.placeholder(tf.float32, [None, env.num_actions])
            if use_tf12_api:
                prediction_readout = tf.reduce_sum(
                    tf.transpose(tf.multiply(
                    tf.transpose(pi.predictions, perm=[1,2,3,0,4]), self.action),
                    perm=[3,0,1,2,4]), reduction_indices=[4])
            else:
                prediction_readout = tf.reduce_sum(
                    tf.transpose(tf.mul(
                    tf.transpose(pi.predictions, perm=[1,2,3,0,4]), self.action),
            ...
            if use_tf12_api:
                self.pixloss_sum = tf.summary.scalar("model/feature_loss", self.prediction_loss / bs)
                self.pixelval = tf.summary.scalar("model/feature_value", self.avg_prediction)
                self.pixelmax = tf.summary.scalar("model/feature_max", self.max_prediction )
            else:
                self.pixloss_sum = tf.scalar_summary("model/feature_loss", self.prediction_loss / bs)
                self.pixelval = tf.scalar_summary("model/feature_value", self.avg_prediction)
                self.pixelmax = tf.scalar_summary("model/feature_max", self.max_prediction )
...
"""
        ...
        if len(self.runner.policy.replay_memory) > 100:
            ...
            while not terminal and bs < 20:
                featbatch.append(self.runner.policy.replay_memory[starting_pos + bs])
                bs += 1
                terminal = featbatch[-1][3]

            last_states = [m[0] for m in featbatch]
            last_actions = [m[5] for m in featbatch]
            last_rewards = [m[6] for m in featbatch]
            rewards = [m[2] for m in featbatch]
            feat_changes = [m[7] for m in featbatch]
            actions = [m[1] for m in featbatch]
            start_features = featbatch[0][4]
            states = [m[8] for m in featbatch]

            featfeed_dict = {self.local_network.x: states,
                                      ...
                                      self.local_network.bs: len(featbatch)
                                        }

            featfeed_dict.update(zip(self.local_network.var_list, self.local_network.target_weights))

            prediction_values = sess.run(self.local_network.predictions, feed_dict=featfeed_dict)

            max_prediction_value = np.max(prediction_values, axis=4)[-1]

            pred_targets = []
            pred_target_shape = np.shape(feat_changes)[1:]
            for i in range(len(featbatch)):
                new_pred_target = np.zeros(shape=pred_target_shape)
                for j in range(len(featbatch)-i):
                    new_pred_target += (.99**j)*feat_changes[i+j]
                pred_targets.append(new_pred_target)
                if not featbatch[-1][3]:
                    pred_targets[-1] += (.99**(len(featbatch)-i))*max_prediction_value


            if should_compute_summary:
                pixfetches = [self.pixloss_sum, self.pixelmax, self.pixelval, self.prediction_train_op, self.global_step]
            else:
